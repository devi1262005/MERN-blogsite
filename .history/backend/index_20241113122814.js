...
app.get("/get_all_stories", authenticateToken, async (req, res) => {
    const{userId}=req.user;
    try{
        const blogStories= await BlogStory.find({userId: userId}).sort({
            isFavorite: -1,
        });
...
app.get("/image_upload", upload.single("image"), async (req, res) => {
    try{
        if(!req.files){
            return res
               .status(400)
               .json({error:true,message:"No images were uploaded"});
        }
        const imgUrl=`http://localhost:8000/uploads/${req.files.filename}`;
...
app.delete("/delete_image", async(req,res) => {
    ...
    try{
        const filename=pathbasename(imgUrl);
        const filePath=path.join(_dirname, 'uploads',filename);
        if(fs.existsSync(filePath)){
            fs.unlinkSync(filePath);
            res.status(200).json({message:"Image deleted successfully"});
        }else{
            res.status(404).json({message:"Image did not get deleted successfully"});
        }
    } catch(error){
