...
app.get("/get_all_stories", authenticateToken, async (req, res) => {
    const { userId } = req.user;
    try {
        const blogStories = await BlogStory.find({ userId: userId }).sort({ isFavourite: -1 });
...
app.post('/edit_story/:userId', authenticateToken, async (req, res) => {
    const { id } = req.params;
    const { title, story, visitedLocation, imageUrl, visitedDate } = req.body;
...
app.delete('/delete_story/:id', authenticateToken, async (req, res) => {
    const{id}=req.params;
    const{userId}=req.user;
    try{
        const blogStory=await BlogStory.findOne({_id:id, userId:userId});
        ...
        await blogStory.deleteOne({id:id,userId:userId});
        const imgUrl=blogStory.imgUrl;
        const filename=path.basename(imgUrl);
        const filePath=path.join(_dirnname,'uploads',filename);
...
app.put("/update_is_favorite/:id",authenticateToken, async(req,res) => {
    const{id} = req.params;
    ...
    try{
        ...
        await blogStory.save();
        res.status(200).json({success:true, message:"Favorite Updated"});
    } catch(error){
        res.status(500).json({success:false, message:error.message});
    }
})

app.get('/blog-stories/filter', authenticateToken, async (req, res) => {
    const { startDate, endDate } = req.query;
    const userId = req.user.userId; // Extract the userId from the JWT payload
    ...
    try {
        ...
        const filteredStories = await BlogStory.find({
            userId: new mongoose.Types.ObjectId(userId), // Cast userId to ObjectId
...
app.get('/search', authenticateToken, async (req, res) => {
    ...
    try {
        const searchResults = await BlogStory.find({
            userId: new mongoose.Types.ObjectId(userId), // Cast userId to ObjectId
            $or: [
